...
def accesoDespachante():
    ...
    theme = get_theme()
    return render_template('despachantes.html', sucursales=sucursales, theme=theme)

@app.route('/sucursal/<int:idSucursal>')
def panelDespachante(idSucursal):
    sucursal = Sucursal.query.get_or_404(idSucursal)
    theme = get_theme()
    return render_template('panel_despachante.html', sucursal=sucursal, theme=theme)
...
def registrarPaquete(idSucursal):
    ...
    theme = get_theme()
    return render_template('registrar_paquete.html', sucursal=sucursal, theme=theme)
...
def registrarSalida(idSucursal):
    ...
    theme = get_theme()
    return render_template('registrar_salida.html', sucursal=sucursal, sucursales=sucursales, paquetes=paquetes, theme=theme)
...
def registrarLlegada(idSucursal):
    ...
    theme = get_theme()
    return render_template('registrar_llegada.html', sucursal=sucursal, transportes=transportesPendientes, theme=theme)
...
def toggle_theme():
    theme = 'dark' if get_theme() == 'light' else 'light'
    response = make_response(redirect(request.referrer or url_for('accesoDespachante')))
